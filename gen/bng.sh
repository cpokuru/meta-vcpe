...
if [ "$CUST_ID" -eq 6 ] || [ "$CUST_ID" -eq 7 ] || [ "$CUST_ID" -eq 8 ] || [ "$CUST_ID" -eq 9 ] || [ "$CUST_ID" -eq 20 ] || [ "$CUST_ID" -eq 51 ]; then
    lxc file push $MVCPE/gen/configs/bng-interfaces-cust-$CUST_ID ${container_name}/etc/network/interfaces 1> /dev/null
elif [ "$CUST_ID" -eq 68 ] || [ "$CUST_ID" -eq 69 ]; then
    SINGLE_MULTI_VLAN=single-vlan
    lxc file push $MVCPE/gen/configs/bng-interfaces-cust-$CUST_ID-$SINGLE_MULTI_VLAN ${container_name}/etc/network/interfaces 1> /dev/null
fi

########################################################################################
# Upload bng dhcp config files

lxc file push $MVCPE/gen/configs/dhcpd.conf ${container_name}/etc/dhcp/dhcpd.conf 1> /dev/null
lxc file push $MVCPE/gen/configs/dhcpd6.conf ${container_name}/etc/dhcp/dhcpd6.conf 1> /dev/null
...
lxc file push $MVCPE/gen/configs/dhcpd-notify.sh ${container_name}/etc/dhcpd-notify.sh --mode '755' 1> /dev/null


########################################################################################
# Upload bng radvd config files

lxc file push $MVCPE/gen/configs/radvd ${container_name}/usr/sbin/radvd --mode '755' 1> /dev/null
lxc file push $MVCPE/gen/configs/radvd-init ${container_name}/etc/init.d/radvd --mode '755' 1> /dev/null

if [ "$CUST_ID" -eq 6 ] || [ "$CUST_ID" -eq 7 ] || [ "$CUST_ID" -eq 8 ] || [ "$CUST_ID" -eq 9 ] || [ "$CUST_ID" -eq 20 ]|| [ "$CUST_ID" -eq 51 ]; then
    lxc file push $MVCPE/gen/configs/radvd-cust-$CUST_ID.conf ${container_name}/etc/radvd.conf 1> /dev/null
elif [ "$CUST_ID" -eq 68 ] || [ "$CUST_ID" -eq 69 ]; then
    SINGLE_MULTI_VLAN=single-vlan
    lxc file push $MVCPE/gen/configs/radvd-cust-$CUST_ID-$SINGLE_MULTI_VLAN.conf ${container_name}/etc/radvd.conf 1> /dev/null
fi

########################################################################################
# Upload bng additional config files

lxc file push $MVCPE/gen/configs/ntp.conf ${container_name}/etc/ntp.conf 1> /dev/null

lxc file push $MVCPE/gen/configs/ports.conf ${container_name}/etc/apache2/ports.conf 1> /dev/null
...
lxc file push $MVCPE/gen/configs/DCMresponse.txt ${container_name}/var/www/html/ 1> /dev/null
...
if true; then
    ...
    cat << EOF > $MVCPE/gen/configs/iptables-restore
#!/bin/sh
### BEGIN INIT INFO
...
EOF
    ...
    lxc file push $MVCPE/gen/configs/iptables-restore ${container_name}/etc/init.d/iptables-restore --mode '755' 1> /dev/null

    rm $MVCPE/gen/configs/iptables-restore
...
lxc file push $MVCPE/gen/configs/dnsmasq-instance-erouter0.conf ${container_name}/etc/ 1> /dev/null
lxc file push $MVCPE/gen/configs/dnsmasq-instance-mg0.conf ${container_name}/etc/ 1> /dev/null

lxc file push $MVCPE/gen/configs/dnsmasq-instance-erouter0.hosts ${container_name}/etc/ 1> /dev/null
lxc file push $MVCPE/gen/configs/dnsmasq-instance-mg0.hosts ${container_name}/etc/ 1> /dev/null
...
lxc file push $MVCPE/gen/configs/dnsmasq-instance-erouter0 ${container_name}/etc/init.d/ --mode '755' 1> /dev/null
...
if [ "$CUST_ID" -eq 9 ]; then
    ...
    lxc file push $MVCPE/gen/configs/dnsmasq-instance-mg0 ${container_name}/etc/init.d/ --mode '755' 1> /dev/null
...
lxc file push $MVCPE/gen/configs/mosquitto.conf ${container_name}/etc/mosquitto/ 1> /dev/null
lxc exec ${container_name} -- bash -c "sed -i '/PIDFILE=\/run\/mosquitto\/mosquitto.pid/a mkdir -p /run/mosquitto' /etc/init.d/mosquitto"

lxc file push $MVCPE/gen/usp-controller/usp_msg_1_2_pb2.py ${container_name}/root/ 1> /dev/null
lxc file push $MVCPE/gen/usp-controller/usp_record_1_2_pb2.py ${container_name}/root/ 1> /dev/null
lxc file push $MVCPE/gen/usp-controller/mqtt-usp-client.py ${container_name}/root/ 1> /dev/null


########################################################################################
# iperf3 server

lxc file push $MVCPE/gen/configs/iperf3-server ${container_name}/etc/init.d/ 1> /dev/null
